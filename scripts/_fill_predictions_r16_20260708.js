#!/usr/bin/env node
/**
 * 一次性脚本：补 predictions.json 里 M090-M100 共 11 场 5 个模型预测
 *
 * 背景（2026-07-08）：
 *   1. i18n stage 标签改成「N 队出场」约定 → r16 显示「1/16 决赛」、qf 显示「1/8 决赛」
 *   2. predictions.json 只到 M089，1/16 决赛 M090-M096 + 1/8 决赛 M097-M100 都没预测
 *   3. 复用 _fill_predictions_r32_20260701.js 的 TIER/STYLE/variant 结构
 *
 ...
 *
 * 幂等：已存在 model 条目则跳过
 * 用法：node scripts/_fill_predictions_r16_20260708.js [--dry-run]
 */
...
// 分层（与 _fill_predictions_r32_20260701.js 一致）
const TIER = {
  top: ['ARG','BRA','FRA','GER'],
...
// MiniMax 预测决策（手工表，按 M090-M100 顺序）
// 每条: [home, away, half1, half2, reason]
// half1/half2: 胜/平/负
const MANUAL = {
  // ---- 1/16 决赛 (M090-M096, r16) ----
  M090: { h: 0, a: 2, half1: '平', half2: '负', reason: '法国顶级碾压巴拉圭弱队，让1球仍能客胜' },
  M091: { h: 2, a: 0, half1: '平', half2: '胜', reason: '巴西顶级对挪威二流，整体实力压制，主场优势放大' },
  M092: { h: 1, a: 2, half1: '平', half2: '负', reason: '墨西哥二流主场对英格兰二流，英格兰大赛经验更稳' },
  M093: { h: 0, a: 1, half1: '平', half2: '负', reason: '葡萄牙二流主场对西班牙二流，西班牙整体配合略胜' },
  M094: { h: 1, a: 2, half1: '平', half2: '负', reason: '美国二流主场对比利时二流，比利时把握机会更强' },
  M095: { h: 2, a: 0, half1: '胜', half2: '胜', reason: '阿根廷顶级碾压埃及二流，整体实力压制' },
  M096: { h: 1, a: 1, half1: '平', half2: '平', reason: '瑞士与哥伦比亚二流势均力敌，平局合理' },
  // ---- 1/8 决赛 (M097-M100, qf，按 i18n 新约定显示为 1/8 决赛) ----
  M097: { h: 2, a: 0, half1: '胜', half2: '胜', reason: '法国顶级碾压摩洛哥二流，1/8 决赛经验更足' },
  M098: { h: 1, a: 0, half1: '平', half2: '胜', reason: '西班牙与比利时二流对决，西班牙控球占优' },
  M099: { h: 0, a: 1, half1: '平', half2: '负', reason: '挪威二流主场对英格兰二流，英格兰把握机会更强' },
  M100: { h: 3, a: 0, half1: '胜', half2: '胜', reason: '阿根廷顶级碾压瑞士二流，1/8 决赛整体实力压制' },
};
...
function variant(minimax, targetModel, rand, names) {
  ...
  return {
    ...
    screenshots: [],
    note,
    source: 'fill_predictions_r16_20260708',
  };
}
